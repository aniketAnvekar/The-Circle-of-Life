...
class Agent8:
    ...
    def update(self, predator, prey):
        ...
        if len(options) != 1:
            distances = mp.getShortestDistancesToGoals(self.graph, self.position, options)
        ...
        if len(options) != 1:
            distances = mp.getShortestDistancesToGoals(self.graph, estimated_predator_position, options)
